...
public interface LearnMaterialMapper extends BaseMapper<LearnMaterial> {
     ...
     */
    @Update("UPDATE homeai_learn_material SET del_flag = 0 WHERE id = #{id}")
    ...
    @Select({
            "<script>",
            "SELECT * FROM homeai_learn_material",
            "WHERE del_flag = 1",
            "<if test='title != null and title != \"\"'>AND title LIKE CONCAT('%', #{title}, '%')</if>",
            "ORDER BY create_time DESC",
            "</script>"
    })
    IPage<LearnMaterial> selectRecycleBinPage(Page<LearnMaterial> page, @Param("title") String title);
    ...
    @Delete({
            "<script>",
            "DELETE FROM homeai_learn_material WHERE id IN",
