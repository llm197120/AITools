...
<script lang="ts" name="homeai-learn-category" setup>
  import { ref } from 'vue';
  ...
  const [registerTable, { reload }] = useTable({
    title: '学习分类管理',
    api: (params: any) => learnApi.categoryList(params),
    columns: [
  ...
  const [registerForm, { setFieldsValue, resetFields, submit }] = useForm({
    ...
    schemas: [
      ...
      { field: 'icon', label: '图标', component: 'Input', defaultValue: '📖' },
      { field: 'color', label: '颜色', component: 'Input', defaultValue: '#999' },
  ...
  function handleAdd() {
    ...
    recordId.value = '';
    resetFields();
    setFieldsValue({ icon: '📖', color: '#999', sortOrder: 0, isEnabled: 1 });
    openModal(true);
  }
  ...
  function handleDelete(record: any) {
    createConfirm({
      ...
      onOk: async () => {
        await learnApi.deleteCategory(record.id);
  ...
  async function handleSubmit(values: any) {
    try {
      if (isUpdate.value) {
        await learnApi.editCategory({ id: recordId.value, ...values });
        createMessage.success('编辑成功');
      } else {
        await learnApi.addCategory(values);
